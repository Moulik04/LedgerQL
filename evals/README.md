...
the real database (see §9.1) — most `needs_validation` cases resolved to
...
decide. Add this as the `eval-validate` Makefile target and run it in CI
against a small fixture DB. **Phase 2's baseline number is not valid
until this passes clean (or every REVIEW NEEDED item has been resolved
